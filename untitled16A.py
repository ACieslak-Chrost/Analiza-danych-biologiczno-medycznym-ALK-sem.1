# -*- coding: utf-8 -*-
"""
Created on Sun Mar 29 11:37:33 2026
...
class MedicalAnalyzerGUI:
    ...
    def dodaj_przycisk_profesjonalny(self):
        try:
            ...
            if btn_frame is None:
                return
            self.pro_btn = self._create_button(btn_frame, "🔬 ANALIZA PROFESJONALNA", self.analiza_profesjonalna,
    ...
    def tab1_wczytaj(self):
        ...
        info_frame = tk.LabelFrame(main_frame, text="📋 INFORMACJE O DANYCH",
        ...
        analyze_btn = self._create_button(main_frame, "🚀 PRZEJDŹ DO ANALIZY",
...
"""
    ...
    def tab2_analiza(self):
        ...
        control_frame = tk.LabelFrame(main_frame, text="🎯 PARAMETRY ANALIZY",
        ...
        analyze_one_btn = self._create_button(btn_frame, "📊 ANALIZUJ WYBRANY", self.analizuj_pojedynczy,
            KOLORY["accent1"], font_size=11, padx=20, pady=8)
        analyze_one_btn.pack(side="left", padx=5)

        analyze_all_btn = self._create_button(btn_frame, "📊 ANALIZUJ WSZYSTKIE", self.analizuj_wszystkie,
    ...
    def analizuj_pojedynczy(self):
        ...
        if self.current_mode == "podstawowa":
            ...
            self.tree.insert("", "end", tags=(tag,), values=(1, pretty_name(param), hosp_txt, dom_txt,
                f"{p_raw:.4f}", f"{p_fdr:.4f}", f"{d:.3f}", efekt, stars))
        ...
        if self.current_mode == "podstawowa":
            ...
            for param in self.parametry_kliniczne:
                ...
                wyniki.append({"parametr": param, "etykieta": pretty_name(param),
                    "hosp_txt": f"{hosp.median():.2f} [{hosp.quantile(0.25):.2f}-{hosp.quantile(0.75):.2f}]",
                    "dom_txt": f"{dom.median():.2f} [{dom.quantile(0.25):.2f}-{dom.quantile(0.75):.2f}]",
                    "p_raw": p_raw, "delta": d, "efekt": interpret_cliff_delta(d), "n_hosp": len(hosp), "n_dom": len(dom)})
    ...
    def tab3_kalkulator(self):
        ...
        predict_btn = self._create_button(actions_frame, "🔮 OBLICZ PRAWDOPODOBIEŃSTWO",
            self.oblicz_prawdopodobienstwo_hospitalizacji, KOLORY["accent1"], font_size=11, padx=14, pady=10)
        predict_btn.pack(fill="x", pady=(0, 8))

        clear_btn = self._create_button(actions_frame, "🧹 WYCZYŚĆ", self.wyczysc_kalkulator,
            KOLORY["warning"], font_size=11, padx=14, pady=10)
        clear_btn.pack(fill="x", pady=(0, 12))

        save_model_btn = self._create_button(actions_frame, "💾 ZAPISZ MODEL", self.zapisz_model_do_pliku,
            KOLORY["success"], font_size=10, padx=12, pady=8)
        save_model_btn.pack(fill="x", pady=(0, 6))

        load_model_btn = self._create_button(actions_frame, "📂 WCZYTAJ MODEL", self.wczytaj_model_z_pliku,
            KOLORY["primary"], font_size=10, padx=12, pady=8)
        load_model_btn.pack(fill="x", pady=(0, 6))

        apply_model_btn = self._create_button(actions_frame, "🧾 UŻYJ MODELU NA PLIKU", self.zastosuj_model_do_pliku,
    ...
    def zbuduj_model_hospitalizacji(self):
        ...
        try:
            ...
            pipeline.fit(X, y)
            n_splits = _dynamiczny_n_splits(y)
    ...
    def tab4_wykresy(self):
        ...
        btn_row.pack(fill="x")
        plot_btn = self._create_button(btn_row, "📈 GENERUJ WYKRES", self.rysuj_wykres,
            KOLORY["accent1"], font_size=11, padx=12, pady=8)
        plot_btn.pack(side="left", padx=5)
        roc_multi_btn = self._create_button(btn_row, "📊 ROC WIELU PARAMETRÓW", self.rysuj_roc_porownawcze,
            KOLORY["secondary"], font_size=11, padx=12, pady=8)
        roc_multi_btn.pack(side="left", padx=5)
        dca_btn = self._create_button(btn_row, "📉 DCA", self.rysuj_decision_curve,
            KOLORY["primary"], font_size=11, padx=12, pady=8)
        dca_btn.pack(side="left", padx=5)
        all_plots_btn = self._create_button(btn_row, "🖼️ GENERUJ WSZYSTKIE\nWYKRESY", self.generuj_wszystkie_wykresy,
            KOLORY["warning"], font_size=11, padx=12, pady=8, wraplength=180, justify="center")
        all_plots_btn.pack(side="right", padx=5)
        save_btn = self._create_button(btn_row, "💾 ZAPISZ WYKRES", self.zapisz_wykres,
    ...
    def tab5_raport(self):
        ...
        btn1 = self._create_button(btn_frame, "📊 GENERUJ RAPORT", self.generuj_raport, KOLORY["accent1"], font_size=12, padx=25, pady=12)
        btn1.pack(side="left", padx=10)
        btn2 = self._create_button(btn_frame, "💾 EKSPORTUJ DO CSV", self.export_csv, KOLORY["success"], font_size=12, padx=25, pady=12)
        btn2.pack(side="left", padx=10)
        btn_pdf = self._create_button(btn_frame, "📄 EKSPORTUJ DO PDF", self.eksportuj_pdf, KOLORY["primary"], font_size=12, padx=25, pady=12)
        btn_pdf.pack(side="left", padx=10)
        btn3 = self._create_button(btn_frame, "🔄 ODŚWIEŻ", self.odswiez_raport, KOLORY["warning"], font_size=12, padx=25, pady=12)
...
"""
    ...
    def tab6_info(self):
        ...
        info_text = """
╔══════════════════════════════════════════════════════════════╗
║         ANALIZATOR DANYCH MEDYCZNYCH - WERSJA 13.4          ║
╚══════════════════════════════════════════════════════════════╝
...
"""
    ...
    def tab7_skala(self):
        ...
        gen_btn = self._create_button(btn_frame, "📌 GENERUJ SKALĘ", self.generuj_skale_ryzyka,
            KOLORY["accent1"], font_size=11, padx=20, pady=8)
        gen_btn.pack(side="left", padx=5)

        freeze_btn = self._create_button(btn_frame, "❄️ ZAMROŹ SKALĘ", self.zamroz_skale_ryzyka,
            KOLORY["warning"], font_size=11, padx=20, pady=8)
        freeze_btn.pack(side="left", padx=5)

        compare_btn = self._create_button(btn_frame, "🆚 PORÓWNAJ SKALE", self.porownaj_skale_ryzyka,
            KOLORY["success"], font_size=11, padx=20, pady=8)
        ...
        calc_btn = self._create_button(calc_btn_col, "🧮 OBLICZ WYNIK SKALI", self.oblicz_wynik_skali,
    ...
    def _model_predykcyjny(self, df, zmienne):
        ...
        n_splits = _dynamiczny_n_splits(y_train)
        cv = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)
    ...
    def analiza_profesjonalna(self):
        ...
        try:
            ...
            self._walidacja_zakresow(df_caly).to_csv("walidacja_zakresow.csv", sep=";", index=False)
    ...
    # =========================================================================
    # METODY DLA KALKULATORA
    # =========================================================================
    def zapisz_model_do_pliku(self):
        if self.prediction_pipeline is None or len(self.prediction_features) == 0:
    ...
    def wczytaj_model_z_pliku(self):
    ...
    def zastosuj_model_do_pliku(self):
        if self.prediction_pipeline is None or len(self.prediction_features) == 0:
