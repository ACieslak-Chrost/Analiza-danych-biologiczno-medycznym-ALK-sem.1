...
def dynamiczny_n_splits(y):
    n_min_class = min(np.bincount(y.astype(int)))
...
class MedicalAnalyzerGUI:
    ...
    # ===== SEKCJA 1: STAŁE KLASY =====
    PARAMETRY_KLINICZNE = PARAMETRY_KLINICZNE
    CHOROBY = CHOROBY
    ZMIENNE_OBOWIAZKOWE = ZMIENNE_OBOWIAZKOWE
    ZMIENNE_DODATKOWE = ZMIENNE_DODATKOWE
    ZMIENNE_LOG = ZMIENNE_LOG
    ZAKRESY_BIOLOGICZNE = ZAKRESY_BIOLOGICZNE
    WARTOSCI_KRYTYCZNE = WARTOSCI_KRYTYCZNE
    KOLORY = KOLORY
    ...
    # ===== SEKCJA 3: METODY POMOCNICZE UI =====
    def _create_button(self, parent, text, command, color, **kwargs):
        btn = tk.Button(
            parent, text=text, command=command,
            font=("Helvetica", kwargs.get('font_size', 11), "bold"),
            bg=color, fg="white",
    ...
    def _sprawdz_czy_dane_wczytane(self):
    ...
    def _znajdz_labelframe(self, parent, tekst):
        for child in parent.winfo_children():
            if isinstance(child, tk.LabelFrame) and child.cget("text") == tekst:
                return child
        return None
    
    def _znajdz_frame_w_labelframe(self, labelframe):
    ...
    def _bezpieczne_wykonanie(self, funkcja, *args, komunikat_bledu="Wystąpił błąd"):
        try:
            return funkcja(*args)
        except Exception as e:
            messagebox.showerror("❌ Błąd", f"{komunikat_bledu}:\n{e}")
            return None
    
    # ===== SEKCJA 4: KONFIGURACJA UI =====
    def setup_ui(self):
        self._konfiguruj_styl()
        ...
        if self.current_mode == "profesjonalna":
            self.root.after(100, self._dodaj_przycisk_profesjonalny)
    
    def _konfiguruj_styl(self):
        style = ttk.Style()
        ...
        style.map("TNotebook.Tab", background=[("selected", KOLORY["primary"])],
    ...
    def _utworz_naglowek(self, parent):
        ...
        tk.Label(header_frame, text="📊 ANALIZA PORÓWNAWCZA PACJENTÓW",
                font=("Helvetica", 20, "bold"), bg=KOLORY["primary"], fg="white").pack(expand=True)
        tk.Label(header_frame, text="Przyjęci do szpitala vs wypisani do domu",
                font=("Helvetica", 12), bg=KOLORY["primary"], fg="white").pack(expand=True)
    
    def _utworz_przelacznik_trybu(self, parent):
        mode_frame = tk.Frame(parent, bg=KOLORY["light"], height=50)
        mode_frame.pack(fill="x", pady=(0, 10))
        tk.Label(mode_frame, text="🔧 TRYB ANALIZY:", font=("Helvetica", 12, "bold"),
                bg=KOLORY["light"], fg=KOLORY["dark"]).pack(side="left", padx=20, pady=10)
        
        self.mode_var = tk.StringVar(value="podstawowa")
        mode_basic = tk.Radiobutton(mode_frame, text="📊 PODSTAWOWA",
            variable=self.mode_var, value="podstawowa", font=("Helvetica", 11),
            bg=KOLORY["light"], fg=KOLORY["dark"], selectcolor=KOLORY["light"],
            command=self._zmien_tryb)
        mode_basic.pack(side="left", padx=10, pady=10)
        
        mode_pro = tk.Radiobutton(mode_frame, text="📈 PROFESJONALNA",
            variable=self.mode_var, value="profesjonalna", font=("Helvetica", 11),
            bg=KOLORY["light"], fg=KOLORY["dark"], selectcolor=KOLORY["light"],
            command=self._zmien_tryb)
    ...
    # ===== SEKCJA 5: ZAKŁADKA 1 - WCZYTYWANIE DANYCH =====
    def _tab1_wczytaj(self):
        main_frame = ttk.Frame(self.tab1, padding="30")
        main_frame.pack(fill="both", expand=True)
        
        self._utworz_przyciski_wczytywania(main_frame)
        self._utworz_ramke_info(main_frame)
        self._utworz_przycisk_przejscia(main_frame)
    ...
    def _utworz_przyciski_wczytywania(self, parent):
        button_frame = tk.Frame(parent, bg=KOLORY["bg"])
    ...
    def _utworz_ramke_info(self, parent):
        info_frame = tk.LabelFrame(parent, text="📋 INFORMACJE O DANYCH",
            font=("Helvetica", 14, "bold"), bg=KOLORY["light"], fg=KOLORY["dark"],
            relief="flat", bd=2, padx=20, pady=15)
        info_frame.pack(fill="both", expand=True, pady=30)
        
        text_frame = tk.Frame(info_frame, bg=KOLORY["light"])
        text_frame.pack(fill="both", expand=True)
        
        self.info_text = tk.Text(text_frame, height=15, font=("Courier", 11), bg="white",
            fg=KOLORY["dark"], relief="flat", bd=1, padx=10, pady=10)
    ...
    def _utworz_przycisk_przejscia(self, parent):
        analyze_btn = self._create_button(parent, "🚀 PRZEJDŹ DO ANALIZY",
            lambda: self.notebook.select(self.tab2), KOLORY["success"], font_size=14, padx=40, pady=15)
    ...
    def _wybierz_i_wczytaj_sqlite(self):
        filename = filedialog.askopenfilename(title="Wybierz bazę SQLite",
...
"""
    ...
    # ===== SEKCJA 6: ZAKŁADKA 2 - ANALIZA STATYSTYCZNA =====
    def _tab2_analiza(self):
        main_frame = ttk.Frame(self.tab2, padding="20")
        main_frame.pack(fill="both", expand=True)
        
        control_frame = self._utworz_ramke_parametrow(main_frame)
        self._utworz_tabele_wynikow(main_frame)
        self._utworz_etykiete_statystyk(main_frame)
        self._ustaw_kolumny_tabeli()
    
    def _utworz_ramke_parametrow(self, parent):
        control_frame = tk.LabelFrame(parent, text="🎯 PARAMETRY ANALIZY",
            font=("Helvetica", 12, "bold"), bg=KOLORY["light"], fg=KOLORY["dark"],
            relief="flat", bd=2, padx=20, pady=15)
        ...
        self.param_combo = ttk.Combobox(control_frame, textvariable=self.param_var,
        ...
        analyze_one_btn = self._create_button(btn_frame, "📊 ANALIZUJ WYBRANY", self._analizuj_pojedynczy,
            KOLORY["accent1"], font_size=11, padx=20, pady=8)
        analyze_one_btn.pack(side="left", padx=5)
        
        analyze_all_btn = self._create_button(btn_frame, "📊 ANALIZUJ WSZYSTKIE", self._analizuj_wszystkie,
            KOLORY["success"], font_size=11, padx=20, pady=8)
        analyze_all_btn.pack(side="left", padx=5)
        
        return control_frame
    
    def _utworz_tabele_wynikow(self, parent):
        table_frame = tk.LabelFrame(parent, text="📋 WYNIKI ANALIZY STATYSTYCZNEJ",
            font=("Helvetica", 12, "bold"), bg=KOLORY["light"], fg=KOLORY["dark"],
            relief="flat", bd=2, padx=20, pady=15)
    ...
    def _utworz_etykiete_statystyk(self, parent):
        stats_frame = tk.Frame(parent, bg=KOLORY["light"], height=60)
    ...
    def _ustaw_kolumny_tabeli(self):
        if self.current_mode == "podstawowa":
            ...
            for col, width in [("lp", 50), ("parametr", 220), ("hosp_n", 75), ("hosp_sr", 100),
                               ("hosp_std", 100), ("dom_n", 75), ("dom_sr", 100), ("dom_std", 100),
                               ("p", 100), ("ist", 70)]:
            ...
            for col, width in [("lp", 50), ("parametr", 220), ("hosp_med", 180), ("dom_med", 180),
                               ("p_raw", 90), ("p_fdr", 90), ("delta", 90), ("efekt", 100), ("ist", 80)]:
                self.tree.column(col, width=width, anchor="center")
    ...
    def _analizuj_pojedynczy(self):
        ...
        if self.current_mode == "podstawowa":
            self._wyswietl_wynik_podstawowy(param, hosp, dom)
        else:
            self._wyswietl_wynik_profesjonalny(param, hosp, dom)
    
    def _wyswietl_wynik_podstawowy(self, param, hosp, dom):
        hosp_sr = hosp.mean()
        hosp_std = hosp.std()
        dom_sr = dom.mean()
        dom_std = dom.std()
        ...
        stars, tag = self._okresl_istotnosc(p)
        self.tree.insert("", "end", tags=(tag,), values=(1, pretty_name(param), len(hosp),
            f"{hosp_sr:.2f}", f"{hosp_std:.2f}", len(dom), f"{dom_sr:.2f}", f"{dom_std:.2f}", f"{p:.4f}", stars))
        self.stats_label.config(text=f"✓ Przeanalizowano parametr: {pretty_name(param)} • n(przyjęci)={len(hosp)} • n(wypisani)={len(dom)}")
    
    def _wyswietl_wynik_profesjonalny(self, param, hosp, dom):
        p_raw = stats.mannwhitneyu(hosp, dom, alternative="two-sided").pvalue
        d = cliff_delta(hosp, dom)
        efekt = interpret_cliff_delta(d)
        
        stars, tag = self._okresl_istotnosc(p_raw)
        ...
        self.tree.insert("", "end", tags=(tag,), values=(1, pretty_name(param), hosp_txt, dom_txt,
            f"{p_raw:.4f}", f"{p_raw:.4f}", f"{d:.3f}", efekt, stars))
        self.stats_label.config(text=f"✓ Tryb profesjonalny • {pretty_name(param)} • mediana/IQR + efekt")
    ...
    def _analizuj_wszystkie(self):
        ...
        if self.current_mode == "podstawowa":
            self._analizuj_wszystkie_podstawowa()
        else:
            self._analizuj_wszystkie_profesjonalna()
    
    def _analizuj_wszystkie_podstawowa(self):
        wyniki = []
        for i, param in enumerate(self.PARAMETRY_KLINICZNE, 1):
            hosp, dom = self._przygotuj_dane_dla_parametru(param)
            if hosp is None or dom is None:
            ...
            stars, tag = self._okresl_istotnosc(p)
            self.tree.insert("", "end", tags=(tag,), values=(i, pretty_name(param), len(hosp),
                f"{hosp_sr:.2f}", f"{hosp_std:.2f}", len(dom), f"{dom_sr:.2f}", f"{dom_std:.2f}", f"{p:.4f}", stars))
            wyniki.append({"parametr": param, "etykieta": pretty_name(param), "p_value": p, "istotnosc": stars})
    ...
    def _analizuj_wszystkie_profesjonalna(self):
        wyniki = []
        p_values = []
        ...
        for i, row in df_wyn.iterrows():
            ...
            self.tree.insert("", "end", tags=(tag,), values=(i + 1, row["etykieta"], row["hosp_txt"],
                row["dom_txt"], f"{row['p_raw']:.4f}", f"{row['p_fdr']:.4f}", f"{row['delta']:.3f}", row["efekt"], stars))
    ...
    # ===== SEKCJA 7: ZAKŁADKA 3 - KALKULATOR HOSPITALIZACJI =====
    def _tab3_kalkulator(self):
        main_frame = ttk.Frame(self.tab3, padding="20")
        main_frame.pack(fill="both", expand=True)
        
        top_frame = self._utworz_ramke_kalkulatora(main_frame)
        middle_frame = ttk.Frame(main_frame)
        middle_frame.pack(fill="both", expand=True)
        
        left_frame = self._utworz_ramke_wprowadzania(middle_frame)
        right_frame = self._utworz_ramke_wyniku(middle_frame)
        
        self._zbuduj_pola_kalkulatora()
    
    def _utworz_ramke_kalkulatora(self, parent):
        top_frame = tk.LabelFrame(parent, text="🧮 KALKULATOR PRAWDOPODOBIEŃSTWA HOSPITALIZACJI",
            font=("Helvetica", 12, "bold"), bg=KOLORY["light"], fg=KOLORY["dark"],
            relief="flat", bd=2, padx=20, pady=15)
    ...
    def _utworz_ramke_wprowadzania(self, parent):
        left_frame = tk.LabelFrame(parent, text="📋 WPROWADŹ DANE PACJENTA",
            font=("Helvetica", 12, "bold"), bg=KOLORY["light"], fg=KOLORY["dark"],
            relief="flat", bd=2, padx=15, pady=15)
        ...
        actions_frame = self._utworz_przyciski_kalkulatora(entry_and_actions)
        
        return left_frame
    
    def _utworz_przyciski_kalkulatora(self, parent):
        actions_frame = tk.Frame(parent, bg=KOLORY["light"])
        actions_frame.pack(side="left", fill="y", padx=(12, 0))
        
        predict_btn = self._create_button(actions_frame, "🔮 OBLICZ PRAWDOPODOBIEŃSTWO",
            self._oblicz_prawdopodobienstwo, KOLORY["accent1"], font_size=11, padx=14, pady=10)
        predict_btn.pack(fill="x", pady=(0, 8))
        
        clear_btn = self._create_button(actions_frame, "🧹 WYCZYŚĆ", self._wyczysc_kalkulator,
            KOLORY["warning"], font_size=11, padx=14, pady=10)
        clear_btn.pack(fill="x", pady=(0, 12))
        
        save_model_btn = self._create_button(actions_frame, "💾 ZAPISZ MODEL", self._zapisz_model,
            KOLORY["success"], font_size=10, padx=12, pady=8)
        save_model_btn.pack(fill="x", pady=(0, 6))
        
        load_model_btn = self._create_button(actions_frame, "📂 WCZYTAJ MODEL", self._wczytaj_model,
            KOLORY["primary"], font_size=10, padx=12, pady=8)
        load_model_btn.pack(fill="x", pady=(0, 6))
        
        apply_model_btn = self._create_button(actions_frame, "🧾 UŻYJ MODELU NA PLIKU", self._zastosuj_model_do_pliku,
            KOLORY["warning"], font_size=10, padx=12, pady=8, wraplength=180, justify="center")
        apply_model_btn.pack(fill="x")
        
        return actions_frame
    
    def _utworz_ramke_wyniku(self, parent):
        right_frame = tk.LabelFrame(parent, text="📊 WYNIK I INTERPRETACJA",
            font=("Helvetica", 12, "bold"), bg=KOLORY["light"], fg=KOLORY["dark"],
            relief="flat", bd=2, padx=15, pady=15)
    ...
    def _zbuduj_pola_kalkulatora(self):
        ...
        for param in unikalne_pola:
            self._utworz_pole_kalkulatora(param)
        
        self._utworz_pola_rr()
    
    def _utworz_pole_kalkulatora(self, param):
    ...
    def _zbuduj_model_hospitalizacji(self):
        ...
        try:
            ...
            if len(final_features) == 0:
                self.calc_info_label.config(text="Nie udało się zbudować kalkulatora: brak odpowiednich zmiennych.")
                return
            
            df_cc = df_model[final_features + ["outcome"]].dropna().copy()
            if len(df_cc) < 20:
                self.calc_info_label.config(text=f"Kalkulator niezbudowany: za mało complete-case do modelu (n={len(df_cc)}).")
                return
            
            if df_cc["outcome"].nunique() < 2:
                self.calc_info_label.config(text="Kalkulator niezbudowany: outcome ma tylko jedną klasę.")
                return
            
            X = df_cc[final_features].values
            y = df_cc["outcome"].values
            
            pipeline = Pipeline([("scaler", StandardScaler()), ("logreg", LogisticRegression(max_iter=1000, random_state=42))])
            pipeline.fit(X, y)
            
            n_splits = dynamiczny_n_splits(y)
            ...
            self.prediction_model_info = (f"Model gotowy.\n• complete-case do modelu: n = {len(df_cc)}\n"
                f"• liczba cech: {len(final_features)}\n• cechy: {', '.join([pretty_name(f) for f in final_features])}\n"
            ...
            self.prediction_pipeline = None
            self.calc_info_label.config(text=f"Nie udało się zbudować kalkulatora: {e}")
    ...
    def _zastosuj_model_do_pliku(self):
        ...
        try:
            ...
            # Konwersja parametrów klinicznych
    ...
    # ===== SEKCJA 8: ZAKŁADKA 4 - WYKRESY =====
    def _tab4_wykresy(self):
        main_frame = ttk.Frame(self.tab4, padding="20")
        main_frame.pack(fill="both", expand=True)
        
        control_frame = self._utworz_ramke_wykresow(main_frame)
        plot_frame = self._utworz_ramke_plotu(main_frame)
        
        self._utworz_wykres_canvas(plot_frame)
    
    def _utworz_ramke_wykresow(self, parent):
        control_frame = tk.LabelFrame(parent, text="🎯 WYBIERZ PARAMETR DO WIZUALIZACJI",
            font=("Helvetica", 12, "bold"), bg=KOLORY["light"], fg=KOLORY["dark"],
            relief="flat", bd=2, padx=20, pady=15)
        control_frame.pack(fill="x", pady=(0, 20))
        ...
        plot_btn = self._create_button(btn_row, "📈 GENERUJ WYKRES", self._generuj_wykres,
            KOLORY["accent1"], font_size=11, padx=12, pady=8)
        plot_btn.pack(side="left", padx=5)
        
        roc_multi_btn = self._create_button(btn_row, "📊 ROC WIELU PARAMETRÓW", self._rysuj_roc_porownawcze,
            KOLORY["secondary"], font_size=11, padx=12, pady=8)
        roc_multi_btn.pack(side="left", padx=5)
        
        dca_btn = self._create_button(btn_row, "📉 DCA", self._rysuj_decision_curve,
            KOLORY["primary"], font_size=11, padx=12, pady=8)
        dca_btn.pack(side="left", padx=5)
        
        all_plots_btn = self._create_button(btn_row, "🖼️ GENERUJ WSZYSTKIE\nWYKRESY", self._generuj_wszystkie_wykresy,
            KOLORY["warning"], font_size=11, padx=12, pady=8, wraplength=300, justify="center")
        all_plots_btn.pack(side="right", padx=5)
        
        save_btn = self._create_button(btn_row, "💾 ZAPISZ WYKRES", self._zapisz_wykres,
            KOLORY["success"], font_size=11, padx=12, pady=8)
        save_btn.pack(side="right", padx=5)
        
        return control_frame
    
    def _utworz_ramke_plotu(self, parent):
        plot_frame = tk.LabelFrame(parent, text="📊 WYKRES", font=("Helvetica", 12, "bold"),
            bg=KOLORY["light"], fg=KOLORY["dark"], relief="flat", bd=2, padx=20, pady=15)
        plot_frame.pack(fill="both", expand=True)
        return plot_frame
    
    def _utworz_wykres_canvas(self, parent):
        self.figure = Figure(figsize=(12, 7), dpi=100, facecolor="white")
        self.ax = self.figure.add_subplot(111)
        self.ax.set_facecolor("#f8f9fa")
        self.canvas = FigureCanvasTkAgg(self.figure, master=parent)
        self.canvas.get_tk_widget().pack(fill="both", expand=True)
        toolbar_frame = tk.Frame(parent, bg=KOLORY["light"])
    ...
    # ===== SEKCJA 9: ZAKŁADKA 5 - RAPORT =====
    def _tab5_raport(self):
        main_frame = ttk.Frame(self.tab5, padding="20")
        main_frame.pack(fill="both", expand=True)
        
        self._utworz_przyciski_raportu(main_frame)
        self._utworz_ramke_raportu(main_frame)
    
    def _utworz_przyciski_raportu(self, parent):
        btn_frame = tk.Frame(parent, bg=KOLORY["light"])
    ...
    def _utworz_ramke_raportu(self, parent):
        report_frame = tk.LabelFrame(parent, text="📋 RAPORT KOŃCOWY", font=("Helvetica", 14, "bold"),
            bg=KOLORY["light"], fg=KOLORY["dark"], relief="flat", bd=2, padx=20, pady=15)
    ...
    def _odswiez_raport(self):
        ...
        raport = self._generuj_tresc_raportu()
        self.report_text.insert("1.0", raport)
    
    def _generuj_tresc_raportu(self):
        if self.current_mode == "podstawowa":
            return self._generuj_raport_podstawowy()
        else:
            return self._generuj_raport_profesjonalny()
    
    def _generuj_raport_podstawowy(self):
        wyniki = []
        istotne = []
        for param in self.PARAMETRY_KLINICZNE:
            hosp, dom = self._przygotuj_dane_dla_parametru(param)
...
"""
    ...
    def _generuj_raport_profesjonalny(self):
        wyniki = []
        p_values = []
        for param in self.PARAMETRY_KLINICZNE:
            hosp, dom = self._przygotuj_dane_dla_parametru(param)
...
"""
    ...
    # ===== SEKCJA 10: ZAKŁADKA 6 - O PROGRAMIE =====
    def _tab6_info(self):
        ...
        info_text = """
╔══════════════════════════════════════════════════════════════╗
║         ANALIZATOR DANYCH MEDYCZNYCH - WERSJA 14.0          ║
╚══════════════════════════════════════════════════════════════╝
...
"""
    ...
    # ===== SEKCJA 11: ZAKŁADKA 7 - SKALA RYZYKA =====
    def _tab7_skala(self):
        main_frame = ttk.Frame(self.tab7, padding="20")
        main_frame.pack(fill="both", expand=True)
        
        top_frame = self._utworz_ramke_skali_top(main_frame)
        middle_frame = ttk.Frame(main_frame)
        middle_frame.pack(fill="both", expand=True)
        
        left_frame = self._utworz_ramke_skali_tabeli(middle_frame)
        right_frame = self._utworz_ramke_skali_kalkulatora(middle_frame)
    
    def _utworz_ramke_skali_top(self, parent):
        top_frame = tk.LabelFrame(parent, text="📌 SKALA RYZYKA HOSPITALIZACJI",
            font=("Helvetica", 12, "bold"), bg=KOLORY["light"], fg=KOLORY["dark"],
            relief="flat", bd=2, padx=20, pady=15)
        top_frame.pack(fill="x", pady=(0, 15))
        ...
        gen_btn = self._create_button(btn_frame, "📌 GENERUJ SKALĘ", self._generuj_skale_ryzyka,
            KOLORY["accent1"], font_size=11, padx=20, pady=8)
        gen_btn.pack(side="left", padx=5)
        
        freeze_btn = self._create_button(btn_frame, "❄️ ZAMROŹ SKALĘ", self._zamroz_skale_ryzyka,
            KOLORY["warning"], font_size=11, padx=20, pady=8)
        freeze_btn.pack(side="left", padx=5)
        
        compare_btn = self._create_button(btn_frame, "🆚 PORÓWNAJ SKALE", self._porownaj_skale_ryzyka,
            KOLORY["success"], font_size=11, padx=20, pady=8)
        compare_btn.pack(side="left", padx=5)
        
        return top_frame
    
    def _utworz_ramke_skali_tabeli(self, parent):
        left_frame = tk.LabelFrame(parent, text="📋 AKTUALNA SKALA",
            font=("Helvetica", 12, "bold"), bg=KOLORY["light"], fg=KOLORY["dark"],
            relief="flat", bd=2, padx=15, pady=15)
        ...
        for col, width in [("lp", 50), ("parametr", 180), ("kierunek", 90), ("prog", 90), ("punkty", 70), ("auc", 70)]:
            self.scale_tree.column(col, width=width, anchor="center")
    ...
    def _utworz_ramke_skali_kalkulatora(self, parent):
        right_frame = tk.LabelFrame(parent, text="🧮 KALKULATOR SKALI + PORÓWNAJ",
            font=("Helvetica", 12, "bold"), bg=KOLORY["light"], fg=KOLORY["dark"],
            relief="flat", bd=2, padx=15, pady=15)
        right_frame.pack(side="left", fill="both", expand=True)
        ...
        calc_btn = self._create_button(calc_btn_col, "🧮 OBLICZ WYNIK SKALI", self._oblicz_wynik_skali,
            KOLORY["primary"], font_size=11, padx=18, pady=10)
    ...
    def _zbuduj_pola_skali(self):
        ...
        if "MAP" in params:
            self._utworz_pola_rr_skali()
    
    def _utworz_pola_rr_skali(self):
    ...
    def _wybierz_kandydatow_do_skali(self):
        kandydaci = ["crp(0-0,5)", "troponina I (0-7,8))", "SpO2", "HGB(12,4-15,2)",
            "hct(38-45)", "kreatynina(0,5-1,2)", "MAP", "wiek"]
    ...
    def _generuj_skale_ryzyka(self):
        ...
        for param in kandydaci:
            ...
            try:
                ...
                wyniki.append({"parametr": param, "etykieta": pretty_name(param),
                    "p_value": p, "auc": auc_val, "kierunek": kierunek, "prog": prog})
            except Exception:
                continue
    ...
    def _odswiez_tabele_skali(self):
        ...
        for i, row in scale_df.iterrows():
            self.scale_tree.insert("", "end", values=(i + 1, row["etykieta"], row["kierunek"],
                f"{row['prog']:.2f}", int(row["punkty"]), f"{row['auc']:.3f}"))
    ...
    def _porownaj_skale_ryzyka(self):
        ...
        if self.scale_current_df is not None and len(self.scale_current_df) > 0:
            ...
            for _, row in self.scale_current_df.iterrows():
                tekst.append(f"• {row['etykieta']} | {row['kierunek']} | próg {row['prog']:.2f} | {int(row['punkty'])} pkt")
    ...
    def _sprawdz_wartosci_krytyczne_skali(self, dane):
        alerty = []
        for param, reguly in self.WARTOSCI_KRYTYCZNE.items():
    ...
    def _oblicz_wynik_skali(self):
        ...
        for _, rule in scale_df.iterrows():
            ...
            if czy_spelnia and poza_norma:
                suma += int(rule["punkty"])
                trafienia.append(f"• {rule['etykieta']}: {val:.2f} ({rule['kierunek']} niż {rule['prog']:.2f}, poza normą) → +{int(rule['punkty'])} pkt")
    ...
    def _zmien_tryb(self):
        ...
        if self.current_mode == "profesjonalna":
            self._dodaj_przycisk_profesjonalny()
    ...
    def _dodaj_przycisk_profesjonalny(self):
        try:
            self._usun_przycisk_profesjonalny()
            control_frame = self._znajdz_labelframe(self.tab2, "🎯 PARAMETRY ANALIZY")
            if control_frame is None:
                return
            btn_frame = self._znajdz_frame_w_labelframe(control_frame)
            if btn_frame is None:
                return
            self.pro_btn = self._create_button(btn_frame, "🔬 ANALIZA PROFESJONALNA", self._analiza_profesjonalna,
                KOLORY["warning"], font_size=11, padx=20, pady=8)
    ...
    def _model_z_redukcja(self, df, zmienne):
        ...
        priorytety = {"wiek": 10, "log_crp(0-0,5)": 9, "SpO2": 8, "log_kreatynina(0,5-1,2)": 7,
            "MAP": 6, "log_troponina I (0-7,8))": 5, "HGB(12,4-15,2)": 4}
    ...
    def _generuj_raport_tekstowy(self, top5, wyn_model_glowny, pred, epv_ok):
        ...
        if pred is not None:
            lines.append("", f"Model predykcyjny: AUC = {pred['auc']:.3f} (95% CI {pred['auc_ci_low']:.3f}-{pred['auc_ci_high']:.3f})")
        
        lines.append("", "OGRANICZENIA:", "- Analiza oparta na complete-case analysis", "- Progi kliniczne mają charakter eksploracyjny")
        if not epv_ok:
    ...
    def _analiza_profesjonalna(self):
        ...
        messagebox.showinfo("🔬 Analiza profesjonalna", "Rozpoczynam pełną analizę publikacyjną.\nTo może potrwać chwilę.")
        
        try:
            df_caly = self.df.copy()
            
            self._raport_brakow_pro(df_caly).to_csv("raport_brakow.csv", sep=";", index=False)
            self._walidacja_zakresow_pro(df_caly).to_csv("walidacja_zakresow.csv", sep=";", index=False)
            self._tabela_1_pro(df_caly).to_csv("tabela_1_publikacyjna.csv", sep=";", index=False)
            ...
            self._missingness_top_pro(top5).to_csv("missingness_top5.csv", sep=";", index=False)
            self._progi_kliniczne_pro(df_caly, top5).to_csv("progi_kliniczne_eksploracyjne.csv", sep=";", index=False)
            ...
            messagebox.showinfo("✅ Sukces", "Analiza profesjonalna zakończona!\n\nWygenerowano pliki wynikowe w katalogu roboczym.")
