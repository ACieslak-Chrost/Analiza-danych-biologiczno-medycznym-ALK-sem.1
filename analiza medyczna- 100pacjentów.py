...
import numpy as np
import matplotlib
matplotlib.use('Agg')  # DODANE - wyłącza okna, tylko zapis
...
# =============================================================================
# 3b. INDYWIDUALNE WYKRESY DLA KAŻDEGO PARAMETRU
# =============================================================================

print("\n✓ Tworzenie indywidualnych wykresów dla każdego parametru...")
...
for param in parametry_kliniczne:
    if param in df_hosp.columns and param in df_dom.columns:
        ...
        if len(hosp) > 0 and len(dom) > 0:
            ...
            plt.scatter(x_hosp, hosp, alpha=0.6, color='darkred', s=40)
            plt.scatter(x_dom, dom, alpha=0.6, color='darkblue', s=40)
            ...
            plt.title(p_text, fontsize=14, fontweight='bold')
            plt.ylabel('Wartość')
            plt.grid(True, alpha=0.3)
            
            # Statystyki opisowe
            text = f"Przyjęci: n={len(hosp)}, śr={hosp.mean():.2f}±{hosp.std():.2f}\nOdesłani: n={len(dom)}, śr={dom.mean():.2f}±{dom.std():.2f}"
            plt.figtext(0.15, 0.02, text, fontsize=9, bbox=dict(facecolor='white', alpha=0.8))
            
            # Zapisz do pliku
            nazwa_pliku = f'wykres_{param}_porownanie.png'
            # Zamień niedozwolone znaki w nazwie pliku
            nazwa_pliku = nazwa_pliku.replace('(', '').replace(')', '').replace(',', '_').replace('-', '_').replace(' ', '_')
...
print(f"✓ Utworzono {len(parametry_kliniczne)} indywidualnych wykresów")
# =============================================================================
...
plt.suptitle('RYSUNEK 9: CHOROBY WSPÓŁISTNIEJĄCE', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('wykres9_choroby.png', dpi=150, bbox_inches='tight')
# USUNIĘTO plt.show()

# =============================================================================
# 13. WYKRES 10: PROFIL PACJENTA (RADAR CHART) - pierwsza wersja
# =============================================================================

print("\n✓ Wykres 10: Profil pacjenta (radar chart)")

parametry_profil1 = ['SpO2', 'HGB(12,4-15,2)', 'crp(0-0,5)', 'wiek']
df_profil = pd.DataFrame()

for param in parametry_profil1:
    if param in df_hosp.columns:
...
if len(df_profil) > 0:
    ...
    ax.set_ylim(0, 100)
    ax.set_title('RYSUNEK 10: PROFIL PACJENTA (wartości znormalizowane)', fontweight='bold', pad=20)
    ax.legend(loc='upper right', bbox_to_anchor=(1.1, 1.1))
    ax.grid(True)
    
    plt.tight_layout()
    plt.savefig('wykres10_profil_podstawowy.png', dpi=150, bbox_inches='tight')
    # USUNIĘTO plt.show()

# =============================================================================
# 14. WYKRES 11: PROFIL PACJENTA (z MAP)
# =============================================================================

print("\n✓ Wykres 11: Profil pacjenta (z MAP)")

parametry_profil2 = ['MAP', 'SpO2', 'HGB(12,4-15,2)', 'crp(0-0,5)', 'wiek']
df_profil2 = pd.DataFrame()

for param in parametry_profil2:
    if param in df_hosp.columns:
        wszystkie = pd.concat([df_hosp[param], df_dom[param]]).dropna()
        min_val = wszystkie.min()
        max_val = wszystkie.max()
        ...
        df_profil2[param] = [hosp_norm, dom_norm]

if len(df_profil2) > 0:
    fig, ax = plt.subplots(figsize=(8, 8), subplot_kw=dict(projection='polar'))
    
    categories = list(df_profil2.columns)
    N = len(categories)
    angles = [n / float(N) * 2 * pi for n in range(N)]
    angles += angles[:1]
    
    # Hospitalizowani
    values = df_profil2.iloc[0].values.flatten().tolist()
    values += values[:1]
    ax.plot(angles, values, 'o-', linewidth=2, color='red', label='Hospitalizowani')
    ax.fill(angles, values, alpha=0.25, color='red')
    
    # Do domu
    values = df_profil2.iloc[1].values.flatten().tolist()
    values += values[:1]
    ax.plot(angles, values, 'o-', linewidth=2, color='blue', label='Do domu')
    ax.fill(angles, values, alpha=0.25, color='blue')
    ...
    ax.set_xticklabels(categories)
    ax.set_ylim(0, 100)
    ax.set_title('RYSUNEK 11: PROFIL PACJENTA (100 = najlepiej)', fontweight='bold', pad=20)
    ax.legend(loc='upper right', bbox_to_anchor=(1.1, 1.1))
    ax.grid(True)
    
    plt.tight_layout()
    plt.savefig('wykres11_profil_z_MAP.png', dpi=150, bbox_inches='tight')
    # =============================================================================
# 15. PODSUMOWANIE KOŃCOWE - CZYNNIKI RYZYKA HOSPITALIZACJI
# =============================================================================

print("\n" + "="*80)
print("15. PODSUMOWANIE - CZYNNIKI RYZYKA HOSPITALIZACJI")
print("="*80)
...
df_podsum = pd.DataFrame(wyniki_podsumowanie)
df_podsum = df_podsum.sort_values('p_value')

# Czynniki istotne statystycznie
istotne = df_podsum[df_podsum['p_value'] < 0.05]
...
for _, row in istotne.iterrows():
    ...
    print(f"  {kolor} {row['parametr']}: {kierunek} u hospitalizowanych")
    print(f"     (hosp: {row['hosp_sr']:.2f} vs dom: {row['dom_sr']:.2f}, "
          f"p={row['p_value']:.4f}, n={int(row['hosp_n'])}/{int(row['dom_n'])})\n")
...
print(f"  • Hospitalizowani: {len(df_hosp)} pacjentów")
print(f"  • Do domu: {len(df_dom)} pacjentów")
print(f"  • Łącznie: {len(df_hosp) + len(df_dom)} pacjentów")
print(f"  • Parametry istotne statystycznie: {len(istotne)}")
print(f"  • Parametry wysoce istotne (p<0.01): "
      f"{len(df_podsum[df_podsum['p_value'] < 0.01])}")
print(f"  • Parametry bez różnic: {len(df_podsum) - len(istotne)}")

# Ranking parametrów
print("\n📈 RANKING PARAMETRÓW WG ISTOTNOŚCI:")
print("-" * 60)
print(f"{'Lp':<3} {'Parametr':<25} {'p-value':<10} {'Istot.':<6} {'Różnica':<10}")
print("-" * 60)

for i, row in df_podsum.iterrows():
    lp = i + 1
    ...
    znak = "+" if row['roznica'] > 0 else ""
    print(f"{lp:<3} {row['parametr'][:25]:<25} {row['p_value']:<10.4f} "
          f"{stars:<6} {znak}{row['roznica']:<+8.2f}")

# =============================================================================
# 16. ANALIZA CHORÓB WSPÓŁISTNIEJĄCYCH
# =============================================================================

print("\n" + "="*80)
print("16. ANALIZA CHORÓB WSPÓŁISTNIEJĄCYCH")
print("="*80)
...
for choroba in choroby:
    if choroba in df_hosp.columns and choroba in df_dom.columns:
        hosp_vals = df_hosp[choroba].apply(convert_choroba)
        dom_vals = df_dom[choroba].apply(convert_choroba)
        
        hosp_proc = hosp_vals.mean() * 100 if hosp_vals.count() > 0 else 0
        dom_proc = dom_vals.mean() * 100 if dom_vals.count() > 0 else 0
        hosp_n = hosp_vals.sum() if hosp_vals.count() > 0 else 0
        dom_n = dom_vals.sum() if dom_vals.count() > 0 else 0
        
        # Test chi-kwadrat dla proporcji
        from scipy.stats import chi2_contingency
        tabela = [[hosp_n, hosp_vals.count() - hosp_n],
                  [dom_n, dom_vals.count() - dom_n]]
        
        if hosp_vals.count() > 0 and dom_vals.count() > 0:
            chi2, p_chi, dof, expected = chi2_contingency(tabela)
        else:
            p_chi = 1.0
        
        wyniki_chorob.append({
            'choroba': choroba,
            'hosp_proc': hosp_proc,
            'hosp_n': int(hosp_n),
            'hosp_total': hosp_vals.count(),
            'dom_proc': dom_proc,
            'dom_n': int(dom_n),
            'dom_total': dom_vals.count(),
            'roznica': hosp_proc - dom_proc,
            'p_value': p_chi
        })

print("\n{:<12} {:>15} {:>15} {:>15} {:>10}".format(
    "Choroba", "Hospitalizowani", "Do domu", "Różnica", "p-value"))
print("-" * 70)

for w in wyniki_chorob:
    print("{:<12} {:>4.1f}% ({:2d}/{:2d})  {:>4.1f}% ({:2d}/{:2d})  {:>+5.1f}%   p={:.4f}".format(
        w['choroba'],
        w['hosp_proc'], w['hosp_n'], w['hosp_total'],
        w['dom_proc'], w['dom_n'], w['dom_total'],
        w['roznica'], w['p_value']
    ))
...
print("\n" + "="*80)
print("17. ZAPIS WYNIKÓW")
print("="*80)

# Zapis do CSV
df_podsum.to_csv('wyniki_parametry.csv', sep=';', index=False, decimal=',')
print("✓ Zapisano: wyniki_parametry.csv")

if len(wyniki_chorob) > 0:
    df_choroby = pd.DataFrame(wyniki_chorob)
    df_choroby.to_csv('wyniki_choroby.csv', sep=';', index=False, decimal=',')
...
try:
    ...
    df_podsum.to_sql('wyniki_parametry', conn, if_exists='replace', index=False)
    print("  ✓ Zapisano tabelę: wyniki_parametry")
    
    # Zapisz wyniki chorób
    if len(wyniki_chorob) > 0:
        df_choroby.to_sql('wyniki_choroby', conn, if_exists='replace', index=False)
        print("  ✓ Zapisano tabelę: wyniki_choroby")
    
    # Zapisz metadane
    metadata = pd.DataFrame({
        'data_analizy': [pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')],
        'liczba_hosp': [len(df_hosp)],
        'liczba_dom': [len(df_dom)],
        'liczba_istotnych': [len(istotne)]
    })
...
print("="*80)
print("\nPODSUMOWANIE:")
print(f"  • Dane: {len(df_hosp)} hosp + {len(df_dom)} dom = {len(df_hosp) + len(df_dom)} pacjentów")
print(f"  • Istotne parametry: {len(istotne)}")
print(f"  • Baza SQL: pacjenci.db")
print(f"  • Wyniki CSV: 2 pliki")
print(f"  • Wykresy: 11 plików PNG")
print("="*80)