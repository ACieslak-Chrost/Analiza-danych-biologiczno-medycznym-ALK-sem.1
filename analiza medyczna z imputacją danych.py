# -*- coding: utf-8 -*-
"""
Created on Thu Apr  9 10:09:41 2026
...
Wersja: 18.0 - FINAŁ Z IMPUTACJĄ I STRATEGIĄ MODELU
Autor: Aneta
"""
...
def imputuj_dane_mediana(df, kolumny, grupa=None):
    """
    Imputacja medianą w podziale na grupy (opcjonalnie)
    """
    df_imp = df.copy()
    
    if grupa is not None and grupa in df_imp.columns:
        for col in kolumny:
            if col in df_imp.columns:
                for g in df_imp[grupa].unique():
                    mask = (df_imp[grupa] == g) & (df_imp[col].isna())
                    mediana = df_imp[df_imp[grupa] == g][col].median()
                    if not np.isnan(mediana):
                        df_imp.loc[mask, col] = mediana
    else:
        for col in kolumny:
            if col in df_imp.columns:
                mediana = df_imp[col].median()
                if not np.isnan(mediana):
                    df_imp[col].fillna(mediana, inplace=True)
    
    return df_imp


def sensitivity_analysis_complete_case(df_imp, df_cc, zmienne, outcome='outcome'):
    """
    Porównanie wyników complete-case vs imputacja
    """
    print("\n" + "="*80)
    print("ANALIZA CZUŁOŚCI: COMPLETE-CASE VS IMPUTACJA")
    print("="*80)
    
    print(f"\n  Complete-case: n={len(df_cc)}")
    print(f"  Po imputacji: n={len(df_imp)}")
    print(f"  Różnica: +{len(df_imp)-len(df_cc)} obserwacji")
    
    wyniki_porownania = []
    
    for param in zmienne[:5]:
        if param in df_imp.columns:
            # Complete-case
            hosp_cc = df_cc[df_cc['outcome'] == 1][param].median()
            dom_cc = df_cc[df_cc['outcome'] == 0][param].median()
            
            # Po imputacji
            hosp_imp = df_imp[df_imp['outcome'] == 1][param].median()
            dom_imp = df_imp[df_imp['outcome'] == 0][param].median()
            
            roznica_hosp = abs(hosp_imp - hosp_cc)
            roznica_dom = abs(dom_imp - dom_cc)
            
            print(f"\n  {param}:")
            print(f"    Hospitalizowani: CC={hosp_cc:.2f} vs IMP={hosp_imp:.2f} (różn.={roznica_hosp:.2f})")
            print(f"    Do domu: CC={dom_cc:.2f} vs IMP={dom_imp:.2f} (różn.={roznica_dom:.2f})")
            
            wyniki_porownania.append({
                'parametr': param,
                'hosp_cc': hosp_cc,
                'hosp_imp': hosp_imp,
                'dom_cc': dom_cc,
    ...
    return pd.DataFrame(wyniki_porownania)


# =============================================================================
# STRATEGIE MODELU INFERENCYJNEGO
# =============================================================================

def model_podstawowy(df, outcome='outcome'):
    """
    Model podstawowy: tylko wiek (minimalny)
    """
    print("\n" + "="*80)
    print("MODEL PODSTAWOWY (TYLKO WIEK)")
    print("="*80)
    
    if 'wiek' not in df.columns:
        print("  ✗ Brak wieku w danych")
        return None, None
    
    df_clean = df[['wiek', outcome]].dropna()
    X = sm.add_constant(df_clean['wiek'])
    y = df_clean[outcome]
    ...
    try:
        ...
        or_val = np.exp(model.params['wiek'])
        ci_low, ci_high = np.exp(model.conf_int().loc['wiek'])
        p_val = model.pvalues['wiek']
        
        print(f"\n  Wiek: OR={or_val:.2f} (95% CI: {ci_low:.2f}-{ci_high:.2f}), p={p_val:.4f}")
...
def model_rozszerzony(df, zmienne, outcome='outcome'):
    """
    Model rozszerzony (wiek + wybrane zmienne kliniczne)
    """
    print("\n" + "="*80)
    print("MODEL ROZSZERZONY (WIEK + BIOMARKERY)")
    print("="*80)
...
def model_z_redukcja(df, zmienne, outcome='outcome', max_epv=10):
    """
    Model z automatyczną redukcją przy niskim EPV
    """
    print("\n" + "="*80)
    print("MODEL Z REDUKCJĄ (EPV ≥ 10)")
    print("="*80)
    
    n_events = df[outcome].sum()
    max_pred = int(n_events / max_epv)
    
    print(f"  Maksymalna liczba predyktorów (EPV≥10): {max_pred}")
    
    if len(zmienne) <= max_pred:
        print("  ✓ Liczba predyktorów OK - używam pełnego modelu")
        return model_rozszerzony(df, zmienne, outcome)
    
    # Redukcja: wybierz najważniejsze klinicznie
    priorytety = {
        'wiek': 10,
        'log_crp(0-0,5)': 9,
        'SpO2': 8,
    ...
    print(f"  Wybrane zmienne: {', '.join(wybrane)}")
...
def przygotuj_dane_do_modelu(df_caly, zmienne_obowiazkowe, zmienne_dodatkowe, zmienne_log):
    """
    Przygotowuje dane z transformacjami log
    """
...
def raport_missingness_grupowa(df_hosp, df_dom, top_param):
    """Raport braków w podziale na grupy"""
    print("\n" + "="*70)
...
def analiza_nieliniowosci(df, zmienna, outcome='outcome'):
    """Analiza nieliniowości przez kategoryzację"""
...
def podsumowanie_z_ograniczeniami(model_podst=None, model_rozsz=None, 
                                   model_red=None, model_pred=None, 
                                   progi=None, porownanie=None):
    ...
    print("  • VIF i EPV z diagnostyką")
    print("  • Transformacje log dla biomarkerów")
    print("  • Poprawny pipeline (bez data leakage)")
    print("  • Analiza nieliniowości (kwartyle)")
    print("  • Imputacja danych + sensitivity analysis")
    print("  • Strategie modelu: podstawowy, rozszerzony, z redukcją")
    
    print("\n⚠️ OGRANICZENIA:")
    print("  • Imputacja medianą (najprostsza)")
    print("  • Brak walidacji zewnętrznej")
    print("  • Progi kliniczne eksploracyjne")
    print("  • Założenie liniowości w modelu logistycznym")
    
    if model_podst is not None:
        print("\n📊 MODEL PODSTAWOWY (tylko wiek):")
        print(f"  • OR(wiek) = {model_podst.iloc[0]['OR']:.2f}")
        print(f"  • 95% CI: {model_podst.iloc[0]['CI_95%']}")
    
    if model_rozsz is not None:
        print("\n📊 MODEL ROZSZERZONY:")
        for _, row in model_rozsz.iterrows():
...
def main(sciezka_pliku, id_pacjenta=None):
    ...
    print("\n" + "="*80)
    print("PROFESJONALNA ANALIZA DANYCH MEDYCZNYCH - WERSJA 18.0")
    ...
    # =========================================================================
    # ETAP 7: RAPORT MISSINGNESS
    # =========================================================================
    
    raport_missingness_grupowa(df_hosp, df_dom, top5_fdr)
    
    # =========================================================================
    # ETAP 8: IMPUTACJA I SENSITIVITY ANALYSIS
    # =========================================================================
    
    print("\n" + "="*80)
    print("IMPUTACJA DANYCH")
    print("="*80)
    
    # Complete-case
    df_cc = df_caly.dropna()
    przeplyw['5. Complete-case'] = len(df_cc)
    
    # Imputacja
    df_imp = imputuj_dane_mediana(df_caly, parametry_kliniczne, grupa='outcome')
    przeplyw['6. Po imputacji'] = len(df_imp)
    
    # Sensitivity analysis
    porownanie = sensitivity_analysis_complete_case(
        df_imp, df_cc, top5_fdr
    )
    if porownanie is not None:
        porownanie.to_csv('sensitivity_analysis.csv', sep=';', index=False)
    
    # =========================================================================
    # ETAP 9: PROGI KLINICZNE
    # =========================================================================
    
    progi = progi_kliniczne_z_bootstrapem(df_imp, top5_fdr)
    if progi is not None and len(progi) > 0:
        progi.to_csv('progi_kliniczne_eksploracyjne.csv', sep=';', index=False)
    
    # =========================================================================
    # ETAP 10: TRANSFORMACJE
    # =========================================================================
    ...
    df_model, zmienne_do_modelu = przygotuj_dane_do_modelu(
        df_imp, ZMIENNE_OBOWIAZKOWE, ZMIENNE_DODATKOWE, ZMIENNE_LOG
    )
    
    przeplyw['7. Po transformacjach'] = len(df_model)
    
    # =========================================================================
    # ETAP 11: MODELE INFERENCYJNE (STRATEGIE)
    # =========================================================================
    
    # Model podstawowy (tylko wiek)
    model_podst, wyniki_podst = model_podstawowy(df_model)
    if model_podst is not None:
        wyniki_podst.to_csv('model_podstawowy.csv', sep=';', index=False)
    
    # Model rozszerzony
    model_rozsz, wyniki_rozsz = model_rozszerzony(df_model, zmienne_do_modelu)
    if model_rozsz is not None:
        wyniki_rozsz.to_csv('model_rozszerzony.csv', sep=';', index=False)
        forest_plot_ostateczny(wyniki_rozsz, 'forest_plot_rozszerzony.png')
    
    # Model z redukcją
    model_red, wyniki_red = model_z_redukcja(df_model, zmienne_do_modelu)
    if model_red is not None:
        wyniki_red.to_csv('model_z_redukcja.csv', sep=';', index=False)
        forest_plot_ostateczny(wyniki_red, 'forest_plot_redukcja.png')
    
    # =========================================================================
    # ETAP 12: MODEL PREDYKCYJNY
    # =========================================================================
    ...
    # =========================================================================
    # ETAP 14: RAPORT PRZEPŁYWU
    # =========================================================================
    
    raport_przeplywu_pacjentow(przeplyw)
    
    # =========================================================================
    # ETAP 15: PODSUMOWANIE
    # =========================================================================
    
    podsumowanie_z_ograniczeniami(
        wyniki_podst, wyniki_rozsz, wyniki_red, 
        model_pred, progi, porownanie
    )
