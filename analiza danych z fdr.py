# -*- coding: utf-8 -*-
"""
Created on Thu Apr  9 10:05:01 2026
...
Wersja: 16.0 - FINAŁ ABSOLUTNY - wszystkie błędy poprawione
Autor: Aneta
"""
# Błędy krytyczne:
#✅ FDR JEST RZECZYWIŚCIE UŻYTY! - pełna analiza jednoczynnikowa z multipletests

#✅ Log-transformacje SĄ UŻYWANE! - zmienne w modelu to wersje z log

#✅ Poprawny pipeline sklearn - fit tylko na train, transform na test

#✅ CV w pipeline - brak data leakage

#✅ VIF/EPV w finalnym modelu - pełna diagnostyka

#✅ Multipletests używane - nie tylko import

#🟡 Nowe elementy:
#✅ Raport missingness grupowej - braki w podziale na outcome

#✅ Przepływ pacjentów - każdy krok z liczebnością

#✅ Czyszczenie importów - tylko używane

#✅ Spójność deklaracji i wykonania - FDR opisana i zrobiona
import pandas as pd
import numpy as np
...
def raport_missingness_grupowa(df_hosp, df_dom, zmienne):
    """Raport braków w podziale na grupy"""
    print("\n" + "="*70)
    print("ANALIZA BRAKÓW W PODZIALE NA GRUPY")
    print("="*70)
    
    for param in zmienne[:5]:  # Top 5
...
# =============================================================================
# ANALIZA INFERENCYJNA Z VIF/EPV
# =============================================================================

def analiza_inferencyjna_statsmodels(df, zmienne, outcome='outcome'):
    """
    Model inferencyjny w statsmodels - z VIF i EPV
    """
    print("\n" + "="*80)
        ...
        for _, row in vif_data.iterrows():
            if row['zmienna'] != 'const':
                if row['VIF'] > 10:
                    ...
                    print(f"  • {row['zmienna']}: VIF={row['VIF']:.2f} (umiarkowana)")
                else:
...
def podsumowanie_z_ograniczeniami(model_inf=None, model_pred=None, progi=None):
    ...
    print("  • Korekta FDR dla wielokrotnych porównań (zaimplementowana)")
    ...
    print("  • VIF i EPV dla modelu inferencyjnego")
    ...
    if model_pred is not None:
        ...
        print("  • AUC i kalibracja na zbiorze testowym")
        print("  • Walidacja krzyżowa na zbiorze treningowym")
...
def main(sciezka_pliku, id_pacjenta=None):
    ...
    print("\n" + "="*80)
    print("PROFESJONALNA ANALIZA DANYCH MEDYCZNYCH - WERSJA 16.0")
    ...
    raport_missingness_grupowa(df_hosp, df_dom, parametry_kliniczne)
