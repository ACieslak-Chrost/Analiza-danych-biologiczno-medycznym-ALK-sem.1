# -*- coding: utf-8 -*-
"""
Created on Sun Mar 29 11:09:15 2026
...
Wersja: 13.2 - POPRAWIONE BŁĘDY KALKULATORA I SKALI
Autor: Aneta
"""
...
# =============================================================================
# KONFIGURACJA - STAŁE GLOBALNE (NIEZMIENNE)
# =============================================================================
PARAMETRY_KLINICZNE = [
...
def sprawdz_vif(X, include_constant=False):
    """
    Oblicza VIF dla zmiennych w X.
    Jeśli include_constant=False, pomija stałą (jeśli istnieje).
    """
    X_clean = X.copy()
...
def _dynamiczny_n_splits(y):
    """Oblicza bezpieczną liczbę foldów dla StratifiedKFold."""
...
class MedicalAnalyzerGUI:
    def __init__(self, root):
        ...
        # Atrybuty instancji (zamiast globalnych stałych)
    ...
    # =========================================================================
    # METODY POMOCNICZE UI (UPRASZCZONE - TYLKO DLA PRZYCISKÓW)
    # =========================================================================
    def _create_button(self, parent, text, command, color, **kwargs):
        """Tworzy przycisk z jednolitym stylem i bindingami."""
        btn = tk.Button(
    ...
    def _przygotuj_dane_dla_parametru(self, param):
        """Wspólna logika pobierania i czyszczenia danych dla parametru."""
    ...
    def dodaj_przycisk_profesjonalny(self):
        try:
            ...
            self.pro_btn = self._create_button(
                btn_frame, "🔬 ANALIZA PROFESJONALNA", self.analiza_profesjonalna,
    ...
    def tab1_wczytaj(self):
        ...
        analyze_btn = self._create_button(
            main_frame, "🚀 PRZEJDŹ DO ANALIZY",
            lambda: self.notebook.select(self.tab2),
            KOLORY["success"], font_size=14, padx=40, pady=15
        )
...
"""
    ...
    def tab2_analiza(self):
        ...
        analyze_one_btn = self._create_button(
            btn_frame, "📊 ANALIZUJ WYBRANY", self.analizuj_pojedynczy,
            KOLORY["accent1"], font_size=11, padx=20, pady=8
        )
        analyze_one_btn.pack(side="left", padx=5)

        analyze_all_btn = self._create_button(
            btn_frame, "📊 ANALIZUJ WSZYSTKIE", self.analizuj_wszystkie,
    ...
    def tab3_kalkulator(self):
        ...
        predict_btn = self._create_button(
            actions_frame, "🔮 OBLICZ PRAWDOPODOBIEŃSTWO", self.oblicz_prawdopodobienstwo_hospitalizacji,
            KOLORY["accent1"], font_size=11, padx=14, pady=10
        )
        predict_btn.pack(fill="x", pady=(0, 8))

        clear_btn = self._create_button(
            actions_frame, "🧹 WYCZYŚĆ", self.wyczysc_kalkulator,
            KOLORY["warning"], font_size=11, padx=14, pady=10
        )
        clear_btn.pack(fill="x", pady=(0, 12))

        save_model_btn = self._create_button(
            actions_frame, "💾 ZAPISZ MODEL", self.zapisz_model_do_pliku,
            KOLORY["success"], font_size=10, padx=12, pady=8
        )
        save_model_btn.pack(fill="x", pady=(0, 6))

        load_model_btn = self._create_button(
            actions_frame, "📂 WCZYTAJ MODEL", self.wczytaj_model_z_pliku,
            KOLORY["primary"], font_size=10, padx=12, pady=8
        )
        load_model_btn.pack(fill="x", pady=(0, 6))

        apply_model_btn = self._create_button(
            actions_frame, "🧾 UŻYJ MODELU NA PLIKU", self.zastosuj_model_do_pliku,
            KOLORY["warning"], font_size=10, padx=12, pady=8,
    ...
    def _zbuduj_pola_kalkulatora(self):
        """TWORZY POLA WEJŚCIOWE DLA KALKULATORA - POPRAWIONA WERSJA"""
        ...
        # RR skurczowe - POPRAWA: najpierw tworzymy StringVar, potem używamy
        ...
        ent_rr1 = tk.Entry(row_rr1, textvariable=self.rr_skurczowe_var, font=("Helvetica", 10), width=18)
        ent_rr1.pack(side="left")

        # RR rozkurczowe - POPRAWA: najpierw tworzymy StringVar, potem używamy
        ...
        ent_rr2 = tk.Entry(row_rr2, textvariable=self.rr_rozkurczowe_var, font=("Helvetica", 10), width=18)
    ...
    def tab7_skala(self):
        ...
        gen_btn = self._create_button(
            btn_frame, "📌 GENERUJ SKALĘ", self.generuj_skale_ryzyka,
            KOLORY["accent1"], font_size=11, padx=20, pady=8
        )
        gen_btn.pack(side="left", padx=5)

        freeze_btn = self._create_button(
            btn_frame, "❄️ ZAMROŹ SKALĘ", self.zamroz_skale_ryzyka,
            KOLORY["warning"], font_size=11, padx=20, pady=8
        )
        freeze_btn.pack(side="left", padx=5)

        compare_btn = self._create_button(
            btn_frame, "🆚 PORÓWNAJ SKALE", self.porownaj_skale_ryzyka,
            KOLORY["success"], font_size=11, padx=20, pady=8
        )
        ...
        calc_btn = self._create_button(
            calc_btn_col, "🧮 OBLICZ WYNIK SKALI", self.oblicz_wynik_skali,
    ...
    def _zbuduj_pola_skali(self):
        """TWORZY POLA WEJŚCIOWE DLA SKALI RYZYKA - POPRAWIONA WERSJA"""
        ...
        # Tworzenie pól dla parametrów (bez MAP)
        for param in params:
            if param == "MAP":
                continue

            # POPRAWA: najpierw tworzymy StringVar i zapisujemy w słowniku
            ...
            ent = tk.Entry(row, textvariable=self.scale_input_vars[param], font=("Helvetica", 10), width=18)
            ent.pack(side="left")

        # Obsługa MAP (jeśli jest w skali)
        if "MAP" in params:
            # RR skurczowe - POPRAWA: najpierw tworzymy StringVar
            ...
            # RR rozkurczowe - POPRAWA: najpierw tworzymy StringVar
    ...
    def tab4_wykresy(self):
        ...
        plot_btn = self._create_button(
            btn_row, "📈 GENERUJ WYKRES", self.rysuj_wykres,
            KOLORY["accent1"], font_size=11, padx=12, pady=8
        )
        plot_btn.pack(side="left", padx=5)

        roc_multi_btn = self._create_button(
            btn_row, "📊 ROC WIELU PARAMETRÓW", self.rysuj_roc_porownawcze,
            KOLORY["secondary"], font_size=11, padx=12, pady=8
        )
        roc_multi_btn.pack(side="left", padx=5)

        dca_btn = self._create_button(
            btn_row, "📉 DCA", self.rysuj_decision_curve,
            KOLORY["primary"], font_size=11, padx=12, pady=8
        )
        dca_btn.pack(side="left", padx=5)

        all_plots_btn = self._create_button(
            btn_row, "🖼️ GENERUJ WSZYSTKIE\nWYKRESY", self.generuj_wszystkie_wykresy,
            KOLORY["warning"], font_size=11, padx=12, pady=8,
            wraplength=180, justify="center"
        )
        all_plots_btn.pack(side="right", padx=5)

        save_btn = self._create_button(
            btn_row, "💾 ZAPISZ WYKRES", self.zapisz_wykres,
    ...
    def _rysuj_wykres_na_ax(self, ax, param, plot_type, hosp, dom):
        """Wspólna logika rysowania wykresu na podany Axes."""
        _, p = stats.mannwhitneyu(hosp, dom, alternative="two-sided")
    ...
    def odswiez_raport(self):
        """
        Uwaga: Ten raport jest uproszczoną wersją tego, co generuje 
        analiza_profesjonalna(). Pełne wyniki z modelami regresji, 
        forest plotami i analizą VIF są zapisywane do plików CSV/PNG.
        """
...
"""
    ...
    def tab6_info(self):
        ...
        info_text = """
╔══════════════════════════════════════════════════════════════╗
║         ANALIZATOR DANYCH MEDYCZNYCH - WERSJA 13.2          ║
╚══════════════════════════════════════════════════════════════╝
