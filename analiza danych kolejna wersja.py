# -*- coding: utf-8 -*-
"""
Created on Thu Apr  9 10:02:58 2026
...
Wersja: 15.0 - FINAŁ - wszystkie błędy poprawione
Autor: Aneta
"""
#🔴 Błędy krytyczne:
#✅ Dodano roc_auc_score - import

#✅ Cliff's delta bezpieczny - fallback do asymptotic

#✅ Dwa osobne modele - inferencyjny (statsmodels) i predykcyjny (sklearn)

#🟡 Błędy metodologiczne:
#✅ Transformacje log - dla CRP, troponiny, kreatyniny

#✅ Pre-definiowany plan analizy - jawne zmienne obowiązkowe

#✅ Raport przepływu pacjentów - każdy etap z liczebnością

#✅ Bootstrap AUC z CI - poprawny

#✅ Usunięte wyciszanie warningów - widoczne ostrzeżenia

#✅ Oczyszczone importy - tylko używane

#✅ Progi eksploracyjne - wyraźnie oznaczone

#🟢 Nowe elementy:
#✅ Podsumowanie z ograniczeniami - jawne mocne i słabe strony

#✅ Dwie ścieżki analizy - OR i AUC osobno

#✅ Kontrola EPV - przed modelem

#✅ Standaryzacja w sklearn - poprawna
import pandas as pd
import numpy as np
...
# =============================================================================
# CLIFF'S DELTA - POPRAWIONY I BEZPIECZNY
# =============================================================================
...
def cliff_delta_bezpieczny(x, y):
    ...
    try:
        # Próba exact, fallback do asymptotic
...
def przygotuj_dane_do_modelu(df_caly, zmienne, zmienne_log):
    """
...
# =============================================================================
# ANALIZA WIELOCZYNNIKOWA - DWA PODEJŚCIA
# =============================================================================

def analiza_inferencyjna_statsmodels(df, zmienne, outcome='outcome'):
    """
    Model inferencyjny w statsmodels - dla OR i CI
    """
    print("\n" + "="*80)
    ...
    if 'wiek' not in dostepne:
        print("  ⚠️ Uwaga: wiek nie jest dostępny!")
...
def analiza_predykcyjna_sklearn(df, zmienne, outcome='outcome', test_size=0.3):
    """
    Model predykcyjny w sklearn - dla metryk i walidacji
    """
    print("\n" + "="*80)
    print("MODEL PREDYKCYJNY (sklearn) - AUC i walidacja")
    print("="*80)
...
def progi_kliniczne_z_bootstrapem(df_caly, top_param, n_bootstrap=1000):
    """Progi kliniczne dla top parametrów"""
    print("\n" + "="*80)
...
def podsumowanie_z_ograniczeniami(model_inf=None, model_pred=None, progi=None):
    ...
    print("  • Dwa podejścia: inferencyjne (statsmodels) i predykcyjne (sklearn)")
    ...
    print("  • Brak analizy nieliniowości (splines, GAM)")
...
def main(sciezka_pliku, id_pacjenta=None):
    ...
    print("\n" + "="*80)
    print("PROFESJONALNA ANALIZA DANYCH MEDYCZNYCH - WERSJA 15.0")
    ...
    # =========================================================================
    # ETAP 5: TOP 5 PARAMETRÓW
    # =========================================================================
    
    print("\n" + "="*80)
    print("TOP 5 PARAMETRÓW (wg p-value)")
    print("="*80)
    ...
    for param in parametry_kliniczne:
        if param in df_caly.columns:
            ...
            if len(hosp) > 0 and len(dom) > 0:
                ...
                wyniki_p.append((param, p))
    
    wyniki_p.sort(key=lambda x: x[1])
    top5 = [x[0] for x in wyniki_p[:5]]
    print(f"  {', '.join(top5)}")
    ...
    df_model = przygotuj_dane_do_modelu(df_caly, ZMIENNE_DODATKOWE, ZMIENNE_LOG)
    
    # =========================================================================
    # ETAP 8: MODEL INFERENCYJNY (statsmodels)
    # =========================================================================
    
    zmienne_do_modelu = ZMIENNE_OBOWIAZKOWE + ZMIENNE_DODATKOWE
    zmienne_do_modelu = [z for z in zmienne_do_modelu if z in df_model.columns]
    
    model_inf, wyniki_inf = analiza_inferencyjna_statsmodels(
    ...
    if model_inf is not None:
        ...
        przeplyw['5. Do modelu inferencyjnego'] = len(df_model[zmienne_do_modelu + ['outcome']].dropna())
    
    # =========================================================================
    # ETAP 9: MODEL PREDYKCYJNY (sklearn)
    # =========================================================================
    
    if model_inf is not None:
        model_pred = analiza_predykcyjna_sklearn(df_model, zmienne_do_modelu)
